...
#' @keywords internal
#'
Tmax <- function(Curve = NULL, Scalar = NULL, Factor = NULL, Shape = NULL,
  Image = NULL , Y, timeScale = 0.1, d_out = 0.1, ...) {
    ...
    if (is.null(Shape) == FALSE) {
      ...
      for (k in 1:dim(Shape$X)[length(dim(Shape$X))]) {
        Shape$X[, , , k] <-
          gpagen(Shape$X[, , , k], print.progress = FALSE)$coords
      }
    }
    ...
    hist_imp_nodes <-
      as.matrix(cbind(1, impurete, length(unique(Y$id))))
    ...
    for (p in 1:(length(unique(Y$id)) / 2 - 1)) {
      ...
      for (i in 1:length(unique(id_feuille))) {
        ...
        ### Il faut trouver les moyen d'avoir de manière automatique les indexes
        # de chaque entree
        ...
        for (l in which_feuilles) {
          if (is.element("curve", inputs) == TRUE)
            w_XCurve <- c(w_XCurve, which(Curve$id == l))
          if (is.element("scalar", inputs) == TRUE)
            w_XScalar <- c(w_XScalar, which(Scalar$id == l))
          if (is.element("factor", inputs) == TRUE)
            w_XFactor <- c(w_XFactor, which(Factor$id == l))
        }

        if (length(unique(Y$id[w_Y])) > 1 &
          ...
          if (is.element("curve", inputs) == TRUE)
            Curve_courant <-
              list(
                type = Curve$type,
                X = Curve$X[w_XCurve, ],
                id = Curve$id[w_XCurve],
                time = Curve$time[w_XCurve]
              )
          if (is.element("scalar", inputs) == TRUE) {
            Scalar_courant <-
              list(type = Scalar$type,
                   X = Scalar$X[w_XScalar, ],
                   id = Scalar$id[w_XScalar])
          }
          if (is.element("factor", inputs) == TRUE) {
            Factor_courant <-
              list(type = Factor$type,
                   X = Factor$X[w_XFactor, ],
          ...
          if (Y$type == "factor" ||
              Y$type == "scalar") {
            Y_courant <- list(type = Y$type,
                              Y = Y$Y[w_Y],
                              id = Y$id[w_Y])
          }
          if (Y$type == "shape") {
            Y_courant <- list(type = "shape",
                              Y = Y$Y[, , w_Y],
                              id = Y$id[w_Y])
          }
          if (Y$type == "image") {
            Y_courant <- list(type = "image",
                              Y = Y$Y[w_Y, ],
                              id = Y$id[w_Y])
          }

          # Il nous faut maintenant faire le split sur toutes les differents
          # types :
          ...
          if (is.element("curve", inputs) == TRUE) {
            feuille_split_Curve <-
              var_split(Curve_courant, Y_courant, timeScale = timeScale,
                        d_out = d_out, ...)
            F_SPLIT <-
              rbind(F_SPLIT, c("Curve", feuille_split_Curve$impurete))
            decoupe <- decoupe + 1
          }
          ...
          if (is.element("scalar", inputs) == TRUE) {
            ...
            if (feuille_split_Scalar$Pure == FALSE) {
              F_SPLIT <- rbind(F_SPLIT,
                               c("Scalar", feuille_split_Scalar$impurete))
              decoupe <- decoupe + 1
            }
          }
          ...
          if (is.element("factor", inputs) == TRUE) {
            ...
            if (feuille_split_Factor$Pure == FALSE) {
              F_SPLIT <- rbind(F_SPLIT,
                               c("Factor", feuille_split_Factor$impurete))
              decoupe <- decoupe + 1
            }
          }
          ...
          if (decoupe > 0) {
            ...
            feuille_split <-
              get(paste("feuille_split_", TYPE, sep = "")) #### renvoie
            # l'impurete ainsi que le split ? gauche et a droite

            #if (imp_apres_split<imp_avant_split){

            if (Y$type == "curve") {
              Y_pred[[unique(id_feuille)[i]]] <-
                kmlShape::meanFrechet(
                  as.data.frame(cbind(Y$id[w_Y], Y$time[w_Y], Y$Y[w_Y])), ...)
            }
            ...
            if (Y$type == "shape") {
              Y_pred[[unique(id_feuille)[i]]] <- mshape(
                Y$Y[, , w_Y, drop = FALSE])
            }

            if (Y$type == "factor") {
              #On va renvoyer la classe dominante ::
              Table <- which.max(table(Y$Y[w_Y]))
              Y_pred[[unique(id_feuille)[i]]] <-
                as.factor(attributes(Table)$names)
            }

            if (Y$type == "image") {
              Y_pred[[unique(id_feuille)[i]]] <-
                apply(Y$Y[w_Y, , drop = FALSE], 2, "mean")
            }
            ...
            hist_imp_nodes <-
              rbind(hist_imp_nodes,
                    c(2 * unique(id_feuille)[i],
                      imp_nodes[[2 * unique(id_feuille)[i]]],
                      length(which(feuille_split$split == 1))))
            hist_imp_nodes <-
              rbind(hist_imp_nodes,
                    c(
                      2 * unique(id_feuille)[i] + 1,
                      imp_nodes[[2 * unique(id_feuille)[i] + 1]],
                      length(which(feuille_split$split == 2))
                    ))


            gauche_id <-
              unique(Y$id[w_Y])[which(feuille_split$split == 1)]
            droit_id <-
              unique(Y$id[w_Y])[which(feuille_split$split == 2)]

            V_split <-
              rbind(V_split,
                    c(TYPE, unique(id_feuille)[i], feuille_split$variable))
            wY_gauche <- NULL
            wY_droit <- NULL
            w_gauche <- NULL
            w_droit <- NULL
            ...
            if (X$type == "curve") {
              trajG <-
                as.data.frame(cbind(X$id[w_gauche], X$time[w_gauche],
                                    X$X[w_gauche, feuille_split$variable]))
              trajD <-
                as.data.frame(cbind(X$id[w_droit], X$time[w_droit],
                                    X$X[w_droit, feuille_split$variable]))
            ...
            info_feuilles <-
              hist_imp_nodes[is.element(hist_imp_nodes[, 1],
                                        feuilles_courantes), ]
            impurete <-
              c(impurete, sum(info_feuilles[, 2] * info_feuilles[, 3] /
                      length(unique(Y$id))))
          }
        }
      }
      ...
      if (count_split == 0) {
        ...
        for (q in unique(id_feuille)) {
          ...
          if (Y$type == "curve") {
            Y_pred[[q]] <-
              kmlShape::meanFrechet(data.frame(Y$id[w], Y$time[w], Y$Y[w]), ...)
          }
          if (Y$type == "scalar") {
            Y_pred[[q]] <- mean(Y$Y[w])
          }
